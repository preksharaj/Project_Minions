...
from database.interface.user import security_check
...
def authenticate_request(headers=None, client_ip=''):
    """
    Method to authenticate all incoming requests
    """
    if headers:
        if 'userId' in headers and 'token' in headers and client_ip != '':
                status_code = security_check(headers['userId'], headers['token'], client_ip)
                return status_code
