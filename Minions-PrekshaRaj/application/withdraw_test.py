...
print '######################## VAULT WITHDRAW TEST #####################'
test_body_2 = json_encode( {"coins":
      [
        {"brandId": 2, "coinAmt": 10},
        {"brandId": 3, "coinAmt": 10},
	{"brandId": 10, "coinAmt": 30}
      ]
    }
)
retdata = simple_http_request(TEST_URL1_LOCALHOST,
                              test_body_2, 'POST',
                              '/vault/withdraw',headers={"Authorization":"prek",'userId':dat['userId'],'token': dat['secret_token']}
                              )
print 'status:', retdata.status
mydata = retdata.read()
print 'body:', mydata
...
print '##################### VAULT STATUS AFTER WITHDRAWAL #########################'
